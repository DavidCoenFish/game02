    ...
    , m_device(pDevice)
{
    ...
    for (size_t preallocatePages = 0; preallocateBytes != 0 && preallocatePages < preallocatePageCount; ++preallocatePages)
    {
        ...
        {
            DX::DebugTrace("LinearAllocator failed to preallocate pages (%zu required bytes, %zu pages)\n",
...
void LinearAllocator::FenceCommittedPages(_In_ ID3D12CommandQueue* commandQueue)
{
    ...
    for (auto page = m_usedPages; page != nullptr; page = nextPage)
    {
        ...
        if (page->RefCount() == 1)
        {
            ...
            DX::ThrowIfFailed(commandQueue->Signal(m_fence.Get(), m_fenceCount));
...
LinearAllocatorPage* LinearAllocator::GetNewPage()
{
    ...
    if (FAILED(hr))
    {
        if (hr != E_OUTOFMEMORY)
        {
            DX::DebugTrace("LinearAllocator::GetNewPage resource allocation failed due to unexpected error %08X\n", static_cast<unsigned int>(hr));
        }
        return nullptr;
    }
    ...
    DX::ThrowIfFailed(spResource->Map(0, nullptr, &pMemory));
