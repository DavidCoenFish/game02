...
void RenderTargetBackBuffer::OnDeviceLost()
{
   m_pRenderTargetResource.Reset();
   m_pDepthResource.Reset();
}

void RenderTargetBackBuffer::OnDeviceRestored(
   ...
   )
{
   ...
   m_pDrawSystem->GetBackBufferWidthHeight(
      backBufferWidth,
      backBufferHeight
      );
   ...
   {
      auto pSwapChain = m_pDrawSystem->GetSwapChain();
...
void RenderTargetBackBuffer::StartRender(ID3D12GraphicsCommandList* const pCommandList)
{
    ...
    int width = 0;
    int height = 0;
    m_pDrawSystem->GetBackBufferWidthHeight(width, height);
