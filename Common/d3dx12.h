    ...
    _In_range_(0,D3D12_REQ_SUBRESOURCES-FirstSubresource) UINT NumSubresources)
{
    ...
    ID3D12Device* pDevice = nullptr;
    pDestinationResource->GetDevice(IID_ID3D12Device, reinterpret_cast<void**>(&pDevice));
    pDevice->GetCopyableFootprints(&Desc, FirstSubresource, NumSubresources, 0, nullptr, nullptr, &RowSizeBytes, nullptr);
    pDevice->Release();
...
inline UINT64 UpdateSubresources(
    ...
    D3D12_BOX* const pDestBoxOrNull = nullptr
   )
{
    // Minor validation
    ...
    else
    {
        for (UINT i = 0; i < NumSubresources; ++i)
        {
            ...
            {
               D3D12_BOX srcBox = {
                pDestBoxOrNull->left, // UINT left;
                0, //UINT top;
                0, //UINT front;
                pDestBoxOrNull->right, //UINT right;
                pDestBoxOrNull->bottom - pDestBoxOrNull->top, // UINT bottom;
                1 //UINT back;
               };
               pCmdList->CopyTextureRegion(&Dst, pDestBoxOrNull->left, pDestBoxOrNull->top, 0, &Src, &srcBox);
            }
            else
            {
   ...
   )
{
    ...
    UINT64 Result = UpdateSubresources(pCmdList, pDestinationResource, pIntermediate, FirstSubresource, NumSubresources, RequiredSize, pLayouts, pNumRows, pRowSizesInBytes, pSrcData, pDestBoxOrNull);
